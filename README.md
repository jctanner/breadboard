...
Fullsend is the agentic software-engineering execution path inside Breadboard.
Breadboard supplies the isolated GitHub-compatible forge, Actions scheduler and
runners, token mint, OpenShell gateway, sandbox controller, telemetry services,
and the Fullsend operations dashboard. Fullsend supplies the workflow shim,
role-specific agent workflows, agent configuration, and the CLI that runs the
triage, code, review, fix, and related roles.

The current integrated path is GitHub-first. The `fullsend-dev/triage-target`
repository in the GitHub emulator carries the `.fullsend/` configuration and
the Fullsend workflow shim. Jira and GitLab remain available to Breadboard's
broader pipeline and combined ticket browser, but they do not currently trigger
Fullsend agent runs directly.

The deployed components have these responsibilities:

| Component | Fullsend integration role |
|-----------|---------------------------|
| GitHub Emulator | Stores repositories, issues, pull requests, workflow definitions, workflow runs, and job state; evaluates event triggers and exposes the Actions-compatible API. |
| GitHub Actions runners | Poll for queued jobs, materialize the Actions environment, execute the Fullsend workflow steps, and report logs and conclusions back to the emulator. There are repository, config-repository, and site-wide runner deployments. |
| Fullsend Mint | Exchanges the runner's development OIDC assertion for a role- and repository-scoped GitHub installation token. The local mint is backed by the `fullsend-mint-dev-credentials` Secret. |
| OpenShell Gateway and sandbox controller | Start constrained agent sandboxes and enforce filesystem and network policies. The sandbox can reach only the services permitted by its policy, such as the GitHub emulator and Vertex AI. |
| Fullsend runner image | Contains the Fullsend binary, OpenShell client, Git tooling, and the local runner integration. The build applies the Breadboard compatibility patches before creating the image. |
| Fullsend Dashboard | Reads GitHub Actions runs and jobs plus Kubernetes pod and event state. It is an operations view; it does not receive the source events or dispatch agent work. |
| MLflow and Observatory | Receive optional agent traces, CI artifacts, and verification data when the workflow and environment are configured for telemetry. |

The service relationship is:

```mermaid
graph LR
    operator[Developer or operator] --> fullsend_ui[Fullsend Dashboard<br/>fullsend.local]

    subgraph forge[GitHub Emulator]
        github_api[Repositories, issues, PRs]
    ...
    subgraph execution[Fullsend execution plane]
        ...
        openshell[OpenShell Gateway<br/>openshell-system]
        sandbox[Agent Sandbox Controller<br/>agent-sandbox-system]
    ...
    fullsend_ui -->|read runs, jobs, pods, events| actions
    fullsend_ui -->|read| github_api
    ...
    agent -->|clone, comment, push, update PR| github_api
    agent -->|model calls| vertex
    agent -.->|optional OTLP traces and artifacts| telemetry
```

The event path begins with activity in the GitHub emulator. The emulator
matches the event against the repository's Fullsend workflow, creates the
workflow run, and queues its jobs. The local runners poll the Actions API rather
than receiving a direct callback from the Fullsend dashboard. Inside the job,
the Fullsend composite action requests a short-lived OIDC assertion, exchanges
it with the local mint for the role required by that stage, and then runs the
agent through OpenShell.

In the development deployment, the emulator runner provides a local OIDC
broker backed by the `FULLSEND_DEV_OIDC_TOKEN` environment value. The mint
validates that assertion and returns the role token generated by
`deploy/scripts/18-deploy-fullsend-mint-dev.sh`. Production Fullsend uses its
normal OIDC and cloud minting path; the local path exists so the same workflow
contracts can be exercised without external GitHub or cloud control-plane
dependencies.

The event and webhook boundary is represented by the GitHub Actions event
system. These are the supported Fullsend triggers in the checked-in workflow:
issue opened, edited, or labeled; issue comments; pull request opened,
synchronized, ready-for-review, closed, labeled, or unlabeled; and submitted
pull request reviews. The workflow forwards the raw event payload to the
reusable dispatch workflow, which selects the agent stage and passes the event
context into it.
...
sequenceDiagram
    ...
    GH->>GH: Match workflow trigger and create run
    ...
    Agent->>GH: Read repository and write comments, branches, or PR updates
...
The dashboard observes this flow from the side. It queries the GitHub Actions
API for workflow and job state and the Kubernetes API for Fullsend pods, jobs,
