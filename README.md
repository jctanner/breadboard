...
Fullsend is Breadboard's GitHub-first agent execution path. Breadboard provides
local GitHub Actions runners, token minting, OpenShell sandboxes, and operational
visibility; Fullsend provides the workflow and agent roles. The current demo
runs from the `fullsend-dev/triage-target` repository in the GitHub emulator.

See [docs/fullsend-integration.md](docs/fullsend-integration.md) for the service
map, event and OIDC token flow, and agent sandbox boundaries. Standalone Mermaid
source files are available in `docs/architecture/diagrams/`.
...
See [docs/README.md](docs/README.md) for the documentation index covering architecture, deployment, and reference material.
